...
(macrolet (;; see src/compiler/srctran.lisp
           (with-byte-specifier ((size-var pos-var spec) &body body)
             (once-only ((spec `(handler-case (macroexpand ,spec)
                                  (error ()
                                    (return (values nil t)))))
                         (temp '(gensym)))
               `(if (and (consp ,spec)
...
(defoptimizer (%rotate-byte derive-type) ((count size posn num))
  ...
  (let ((size (sb-c::lvar-type size))
        (posn (sb-c::lvar-type posn)))
    (if (and (numeric-type-p size)
             (numeric-type-p posn)
             (numeric-type-high size)
             (numeric-type-high posn))
...
;; rotate.
#-(or x86 x86-64 ppc arm arm64 riscv loongarch64)
