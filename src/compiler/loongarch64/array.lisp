;;;; array operations for the LoongArch VM
...
(define-vop (make-array-header)
  ...
  (:generator 5
    ...
    (with-fixnum-as-word-index (rank ndescr)
      (inst addi.d ndescr rank (+ (* array-dimensions-offset n-word-bytes)
                                lowtag-mask)))
    (inst s_andi ndescr ndescr (lognot lowtag-mask))
    (pseudo-atomic (pa-flag)
      ...
      (inst s_andi ndescr ndescr (fixnumize array-rank-mask))
      (inst slli.d ndescr ndescr array-rank-position)
      (inst or ndescr ndescr type)
      (inst srli.d ndescr ndescr n-fixnum-tag-bits)
...
(define-vop ()
  ...
  (:generator 6
    (inst ld.bu res x (- (/ array-rank-position n-byte-bits) other-pointer-lowtag))
    (inst addi.d res res 1)
    (inst s_andi res res array-rank-mask)))
         ...
         `(progn
            (define-vop (,refname)
              ...
              (:generator 20
                ...
                (inst srli.d temp index ,bit-shift)
                ;; Load the word in question.
                (inst slli.d temp temp word-shift)
                ...
                (inst s_andi temp index ,(1- elements-per-word))
                ,@(unless (= bits 1)
                    `((inst slli.d temp temp ,(1- (integer-length bits)))))
                ;; Shift the field we need to the low bits of RESULT.
                (inst srl.d result result temp)
                ;; Mask out the field we're interested in.
                (inst s_andi result result ,(1- (ash 1 bits)))
                ;; And fixnum-tag the result.
                (inst slli.d value result n-fixnum-tag-bits)))
            (define-vop (,(symbolicate "DATA-VECTOR-REF-C/" type))
              ...
              (:generator 15
                (multiple-value-bind (word extra) (floor index ,elements-per-word)
                  (loadw result object (+ word vector-data-offset) other-pointer-lowtag)
                  (unless (zerop extra)
                    (inst srli.d result result (* extra ,bits)))
            ...
            (define-vop (,setname)
              ...
              (:generator 25
                ...
                ;; Compute the position of the bitfield we need.
                (inst s_andi shift index ,(1- elements-per-word))
                ,@(unless (= bits 1)
                    `((inst slli.d shift shift ,(1- (integer-length bits)))))
                ;; Clear the target bitfield.
                (unless (and (sc-is value immediate)
                             (= (tn-value value) ,(1- (ash 1 bits))))
                  (inst li temp ,(1- (ash 1 bits)))
                  (inst sll.d temp temp shift)
                ...
                (unless (sc-is value zero)
                  (sc-case value
                    ...
                    (unsigned-reg
                     (inst s_andi temp value ,(1- (ash 1 bits)))))
                  (inst sll.d temp temp shift)
          ...
          (loadw old object (+ word vector-data-offset) other-pointer-lowtag)
                  (unless (and (sc-is value immediate)
                    ...
                    (cond ((= extra ,(1- elements-per-word))
                           (inst slli.d old old ,bits)
                           (inst srli.d old old ,bits))
                          (t
                           (inst li temp
                  ...
                  (sc-case value
                    ...
                    (immediate
                     (let ((value (ash (logand (tn-value value) ,(1- (ash 1 bits)))
                       ...
                       (typecase value
                         (short-immediate
                          (inst s_ori old old value))
                         (t
                          (inst li temp value)
                          (inst or old old temp)))))
                    (unsigned-reg
                     (inst slli.d temp value (* extra ,bits))
                     (inst or old old temp)))
...
(define-atomic-frobber array-atomic-incf/word amadd_db.d * vector-data-offset
