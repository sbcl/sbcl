;;;; allocation VOPs for the LoongArch
        ...
        (pseudo-atomic (pa-flag)
          ...
          (let ((ptr (if (= cons-cells 1)
            ...
            (dotimes (i (1- cons-cells))
              (storew (maybe-load (tn-ref-tn things)) ptr cons-car-slot list-pointer-lowtag)
              (setf things (tn-ref-across things))
              (inst addi.d ptr ptr (pad-data-block cons-size))
  ...
  (:generator 100
    (pseudo-atomic (pa-flag)
      (with-fixnum-as-word-index (words bytes)
        (inst addi.d bytes words (* (1+ vector-data-offset) n-word-bytes)))
      (inst s_andi bytes bytes (lognot lowtag-mask))
...
(define-vop (allocate-vector-on-stack)
  ...
  (:generator 100
    (pseudo-atomic (pa-flag)
      ...
      (inst addi.d temp result (- (* vector-data-offset n-word-bytes)
                                other-pointer-lowtag))
      ;; Zero fill
      (let ((loop (gen-label)))
        (emit-label loop)
        (storew zero-tn temp 0)
        (inst addi.d temp temp n-word-bytes)
...
(define-vop (var-alloc)
  ...
  (:generator 6
    (with-fixnum-as-word-index (extra bytes)
      (inst addi.d bytes extra (* (1- words) n-word-bytes)))
    (inst slli.d header bytes (- (length-field-shift type) word-shift))
    (inst addi.d header header type)
    ...
    (unless nil
      (inst addi.d bytes bytes (* 2 n-word-bytes)))
    (inst s_andi bytes bytes (lognot lowtag-mask))
    (pseudo-atomic (pa-flag)
