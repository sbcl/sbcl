;;;; SAP operations for the LoongArch VM
...
(defun sap-ref (result sap offset signed size)
  (ecase size
    (:byte
     (if signed
         (inst ld.b result sap offset)
         (inst ld.bu result sap offset)))
    (:short
     (if signed
         (inst ld.h result sap offset)
         (inst ld.hu result sap offset)))
    (:word
     (if signed
         (inst ld.w result sap offset)
         (inst ld.wu result sap offset)))

    (:dword
     (inst ld.d result sap offset))
    (:single
...
(defun sap-set (value sap offset size)
  (ecase size
    (:byte
     (inst st.b value sap offset))
    (:short
     (inst st.h value sap offset))
    (:word
     (inst st.w value sap offset))
    (:dword
     (inst st.d value sap offset))
    (:single
             ...
             `(progn
                  ,@(when (member ref-name '(sap-ref-32 sap-ref-64 signed-sap-ref-64
                    ...
                    (multiple-value-bind (load store)
                        (ecase size
                          (:word  (values 'll.w 'sc.w))
                          (:dword (values 'll.d 'sc.d)))
