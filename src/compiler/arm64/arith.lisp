      ...
      (unless fixnum-only
        (assemble (:elsewhere)
          ...
          (unless (fixnum-or-other-pointer-tn-ref-p number-ref t)
            (test-type number tmp-tn error t (other-pointer-lowtag) :value-tn-ref number-ref))
          (loadw tmp-tn number 0 other-pointer-lowtag)
          (inst cmp tmp-tn (bignum-header-for-length 1))
          (inst b :ne ERROR)
          (loadw r number bignum-digits-offset other-pointer-lowtag)
          (inst asr r r (- amount))
          (inst b DONE)))
      (inst tbnz* number 0 (or signed error)))
    (inst asr r number (+ (- amount) n-fixnum-tag-bits))
    done))
