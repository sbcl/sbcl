...
(define-vop (fixnum*=>fixnum)
  ...
  (:args (x :scs (signed-reg))
         (y :scs (signed-reg)))
  (:arg-types tagged-num tagged-num)
  (:temporary (:sc non-descriptor-reg) high low)
  (:results (r :scs (any-reg descriptor-reg)))
  (:note "inline (signed-byte 64) arithmetic")
  (:policy :fast-safe)
  (:vop-var vop)
  (:generator 2
    (let ((error (generate-error-code vop 'sb-kernel::fixnum*-overflow-error x y)))
      (assemble ()
        (inst smulh high x y)
        (inst add low high 1)
        (inst cmp low 1)
        (inst b :hi error)
        (inst mul low x y)
        (inst eor high high low)
        (inst tbnz high 63 error)
        (inst adds r low low)
        (inst b :vs error)))))
