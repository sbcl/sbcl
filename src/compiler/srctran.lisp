...
(deftransform equal ((x y) * *)
  ...
  (let ((x-type (lvar-type x))
    ...
    (flet ((both-csubtypep (type)
           ...
           (non-equal-vector-type-p (type)
             (and (array-type-p type)
                  (neq (array-type-upgraded-element-type type) *wild-type*)
                  (not (csubtypep (array-type-upgraded-element-type type)
                                  (specifier-type '(or character bit)))))))
      (cond
        ((same-leaf-ref-p x y) t)
        ...
        ((types-equal-or-intersect x-type y-type)
         (cond ((some-csubtypep 'number)
                '(eql x y))
               ((or (some-csubtypep '(and array (not vector)))
                    (non-equal-vector-type-p x-type)
                    (non-equal-vector-type-p y-type))
                '(eq x y))
                          ...
                          (and (both-csubtypep 'simple-array)
                               (loop for x-dim in x-dims
                                     ...
                                     thereis (and (integerp x-dim)
                                                  (integerp y-dim)
                                                  (not (= x-dim y-dim))))))))
               (let ((x-et (ctype-array-upgraded-element-type x-type))
                     (y-et (ctype-array-upgraded-element-type y-type)))
