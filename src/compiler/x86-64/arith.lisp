...
(define-vop (overflow-ash-t-signed)
  (:translate overflow-ash)
  (:args (number :scs (any-reg descriptor-reg)))
  (:arg-types (:or t tagged-num) (:constant (integer #.(- 1 sb-vm:n-word-bits) -1)))
  (:arg-refs number-ref)
  (:temporary (:sc unsigned-reg) temp)
  (:info amount type fixnum-only)
  (:results (r :scs (signed-reg)))
  (:result-types tagged-num)
  (:policy :fast-safe)
  (:vop-var vop)
  ...
  (:generator 10
    (let* ((*location-context* (unless (eq type 'fixnum)
                                 type))
           (error (generate-error-code vop 'sb-kernel::ash-overflow2-error number
                                       (encode-immediate-error-arg amount)))
           (signed))
      (unless fixnum-only
        (assemble (:elsewhere)
          start
          (setf signed start)
          (unless (fixnum-or-other-pointer-tn-ref-p number-ref t)
            (test-type number temp error t (other-pointer-lowtag) :value-tn-ref number-ref))
          (inst cmp :qword (object-slot-ea number 0 other-pointer-lowtag) (bignum-header-for-length 1))
          (inst jmp :ne ERROR)
          (loadw r number bignum-digits-offset other-pointer-lowtag)
          (inst sar r (- amount))
          (inst jmp DONE)))
      (generate-fixnum-test number)
      (inst jmp :nz (or signed error))
      (move r number)
      (inst sar r (+ (- amount) n-fixnum-tag-bits)))
    done))
