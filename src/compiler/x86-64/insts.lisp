...
(flet ((emit* (name segment maybe-size dst src lockp opcode allowp)
  ...
  (macrolet ((define (name subop &optional allow-constants &rest rest)
               `(define-instruction ,name (segment maybe-size dst &optional src prefix)
                  (:printer accum-imm ((op ,(dpb subop (byte 3 2) #b0000010))) ,@rest)
                  (:printer reg/mem-imm ((op '(#b1000000 ,subop))) ,@rest)
    ...
    (define .add #b000 nil :default :print-name 'add)
    (define adc #b010)
    (define .sub #b101 nil :default :print-name 'sub)
    (define sbb #b011)
    (define cmp #b111 t)
    (define and #b100)
    (define or  #b001)
...
(define-instruction-macro add (&rest args) `(%add ,@args))
(defun %add (size dst &optional src lockp)
  (cond ((keywordp size)
         (if (eql src 1)
             (inst inc size dst lockp)
             (inst .add size dst src lockp)))
        (t
         (shiftf lockp src dst size)
         (if (eql src 1)
             (inst inc dst lockp)
             (inst .add dst src lockp)))))
(define-instruction-macro sub (&rest args) `(%sub ,@args))
(defun %sub (size dst &optional src lockp)
  (cond ((keywordp size)
         (if (eql src 1)
             (inst dec size dst lockp)
             (inst .sub size dst src lockp)))
        (t
         (shiftf lockp src dst size)
         (if (eql src 1)
             (inst dec dst lockp)
             (inst .sub dst src lockp)))))
