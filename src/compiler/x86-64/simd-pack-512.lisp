...
;;;; AVX-512 VOPs and Intrinsics for 512-bit SIMD Packs

;;; Floating-point arithmetic (single-float, 16 lanes)
(macrolet ((def (name op cost)
             `(progn
                (defknown ,name ((simd-pack-512 single-float) (simd-pack-512 single-float))
                    (simd-pack-512 single-float)
                    (foldable flushable movable))
                (define-vop (,name)
                  (:translate ,name)
                  (:policy :fast-safe)
                  (:args (x :scs (single-avx512-reg) :target dst)
                         (y :scs (single-avx512-reg)))
                  (:arg-types simd-pack-512-single simd-pack-512-single)
                  (:results (dst :scs (single-avx512-reg)))
                  (:result-types simd-pack-512-single)
                  (:generator ,cost
                    (inst ,op dst x y))))))
  (def simd-pack-512-single+   vaddps 2)
  (def simd-pack-512-single-   vsubps 2)
  (def simd-pack-512-single*   vmulps 2)
  (def simd-pack-512-single/   vdivps 8)
  (def simd-pack-512-single-min vminps 2)
  (def simd-pack-512-single-max vmaxps 2))

(defknown simd-pack-512-single-sqrt ((simd-pack-512 single-float))
    (simd-pack-512 single-float)
    (foldable flushable movable))
(define-vop (simd-pack-512-single-sqrt)
  (:translate simd-pack-512-single-sqrt)
  (:policy :fast-safe)
  (:args (x :scs (single-avx512-reg)))
  (:arg-types simd-pack-512-single)
  (:results (dst :scs (single-avx512-reg)))
  (:result-types simd-pack-512-single)
  (:generator 8
    (inst vsqrtps dst x)))

;;; Floating-point arithmetic (double-float, 8 lanes)
(macrolet ((def (name op cost)
             `(progn
                (defknown ,name ((simd-pack-512 double-float) (simd-pack-512 double-float))
                    (simd-pack-512 double-float)
                    (foldable flushable movable))
                (define-vop (,name)
                  (:translate ,name)
                  (:policy :fast-safe)
                  (:args (x :scs (double-avx512-reg) :target dst)
                         (y :scs (double-avx512-reg)))
                  (:arg-types simd-pack-512-double simd-pack-512-double)
                  (:results (dst :scs (double-avx512-reg)))
                  (:result-types simd-pack-512-double)
                  (:generator ,cost
                    (inst ,op dst x y))))))
  (def simd-pack-512-double+   vaddpd 2)
  (def simd-pack-512-double-   vsubpd 2)
  (def simd-pack-512-double*   vmulpd 2)
  (def simd-pack-512-double/   vdivpd 8)
  (def simd-pack-512-double-min vminpd 2)
  (def simd-pack-512-double-max vmaxpd 2))

(defknown simd-pack-512-double-sqrt ((simd-pack-512 double-float))
    (simd-pack-512 double-float)
    (foldable flushable movable))
(define-vop (simd-pack-512-double-sqrt)
  (:translate simd-pack-512-double-sqrt)
  (:policy :fast-safe)
  (:args (x :scs (double-avx512-reg)))
  (:arg-types simd-pack-512-double)
  (:results (dst :scs (double-avx512-reg)))
  (:result-types simd-pack-512-double)
  (:generator 8
    (inst vsqrtpd dst x)))

;;; Integer arithmetic (dword / 32-bit, 16 lanes)
(macrolet ((def (name op cl-el-type vop-type cost)
             `(progn
                (defknown ,name ((simd-pack-512 ,cl-el-type) (simd-pack-512 ,cl-el-type))
                    (simd-pack-512 ,cl-el-type)
                    (foldable flushable movable))
                (define-vop (,name)
                  (:translate ,name)
                  (:policy :fast-safe)
                  (:args (x :scs (int-avx512-reg) :target dst)
                         (y :scs (int-avx512-reg)))
                  (:arg-types ,vop-type ,vop-type)
                  (:results (dst :scs (int-avx512-reg)))
                  (:result-types ,vop-type)
                  (:generator ,cost
                    (inst ,op dst x y))))))
  (def simd-pack-512-ub32+ vpaddd  (unsigned-byte 32) simd-pack-512-ub32 2)
  (def simd-pack-512-sb32+ vpaddd  (signed-byte 32)   simd-pack-512-sb32 2)
  (def simd-pack-512-ub32- vpsubd  (unsigned-byte 32) simd-pack-512-ub32 2)
  (def simd-pack-512-sb32- vpsubd  (signed-byte 32)   simd-pack-512-sb32 2)
  (def simd-pack-512-ub32* vpmulld (unsigned-byte 32) simd-pack-512-ub32 4)
  (def simd-pack-512-sb32* vpmulld (signed-byte 32)   simd-pack-512-sb32 4)
  ;; Integer arithmetic (qword / 64-bit, 8 lanes)
  (def simd-pack-512-ub64+ vpaddq  (unsigned-byte 64) simd-pack-512-ub64 2)
  (def simd-pack-512-sb64+ vpaddq  (signed-byte 64)   simd-pack-512-sb64 2)
  (def simd-pack-512-ub64- vpsubq  (unsigned-byte 64) simd-pack-512-ub64 2)
  (def simd-pack-512-sb64- vpsubq  (signed-byte 64)   simd-pack-512-sb64 2)
  (def simd-pack-512-ub64* vpmullq (unsigned-byte 64) simd-pack-512-ub64 4)
  (def simd-pack-512-sb64* vpmullq (signed-byte 64)   simd-pack-512-sb64 4))

;;; Bitwise logical operations on 512-bit packs
(macrolet ((def (name op)
             `(progn
                (defknown ,name ((simd-pack-512 (unsigned-byte 64))
                ...
                (define-vop (,name)
                  ...
                  (:generator 1
                    (inst ,op dst x y))))))
  (def simd-pack-512-and   vpandd)
  (def simd-pack-512-or    vpord)
  (def simd-pack-512-xor   vpxord)
  (def simd-pack-512-andc1 vpandnd))

;;; Broadcast operations (scalar to 512-bit vector)
(defknown simd-pack-512-broadcast-single (single-float) (simd-pack-512 single-float)
    (foldable flushable movable))
(define-vop (simd-pack-512-broadcast-single)
  (:translate simd-pack-512-broadcast-single)
  (:policy :fast-safe)
  ...
  (:result-types simd-pack-512-single)
  (:generator 2
    (inst vbroadcastss dst x)))

(defknown simd-pack-512-broadcast-double (double-float) (simd-pack-512 double-float)
    (foldable flushable movable))
(define-vop (simd-pack-512-broadcast-double)
  (:translate simd-pack-512-broadcast-double)
  (:policy :fast-safe)
  ...
  (:result-types simd-pack-512-double)
  (:generator 2
    (inst vbroadcastsd dst x)))

(defknown simd-pack-512-broadcast-ub32 ((unsigned-byte 32)) (simd-pack-512 (unsigned-byte 32))
    (foldable flushable movable))
(define-vop (simd-pack-512-broadcast-ub32)
  (:translate simd-pack-512-broadcast-ub32)
  (:policy :fast-safe)
  (:args (x :scs (unsigned-reg signed-reg)))
  (:arg-types unsigned-num)
  (:results (dst :scs (int-avx512-reg)))
  (:result-types simd-pack-512-ub32)
  (:generator 2
    (inst vpbroadcastd dst x)))

(defknown simd-pack-512-broadcast-ub64 ((unsigned-byte 64)) (simd-pack-512 (unsigned-byte 64))
    (foldable flushable movable))
(define-vop (simd-pack-512-broadcast-ub64)
  (:translate simd-pack-512-broadcast-ub64)
  (:policy :fast-safe)
  (:args (x :scs (unsigned-reg signed-reg)))
  (:arg-types unsigned-num)
  (:results (dst :scs (int-avx512-reg)))
  (:result-types simd-pack-512-ub64)
  (:generator 2
    (inst vpbroadcastq dst x)))

;;; Comparison operations (Vector to Opmask)
(macrolet ((def (name cond-kw)
             `(progn
                (defknown ,name ((simd-pack-512 single-float) (simd-pack-512 single-float))
                    simd-pack-512-mask (foldable flushable movable))
                (define-vop (,name)
                  (:translate ,name)
                  (:policy :fast-safe)
                  (:args (x :scs (single-avx512-reg))
                  ...
                  (:result-types simd-pack-512-mask-type)
                  (:generator 3
                    (inst vcmpps ,cond-kw dst x y))))))
  (def simd-pack-512-single=  :eq)
  (def simd-pack-512-single/= :neq)
  (def simd-pack-512-single<  :lt)
  (def simd-pack-512-single<= :le)
  (def simd-pack-512-single>  :gt)
  (def simd-pack-512-single>= :ge))
...
(macrolet ((def (name cond-kw)
             `(progn
                ...
                (define-vop (,name)
                  ...
                  (:result-types simd-pack-512-mask-type)
                  (:generator 3
                    (inst vcmppd ,cond-kw dst x y))))))
  (def simd-pack-512-double=  :eq)
  (def simd-pack-512-double/= :neq)
  (def simd-pack-512-double<  :lt)
  (def simd-pack-512-double<= :le)
  (def simd-pack-512-double>  :gt)
  (def simd-pack-512-double>= :ge))

(macrolet ((def (name op cl-el-type vop-type)
             `(progn
                (defknown ,name ((simd-pack-512 ,cl-el-type) (simd-pack-512 ,cl-el-type))
                    simd-pack-512-mask
                    (foldable flushable movable))
                (define-vop (,name)
                  ...
                  (:generator 3
                    (inst ,op dst x y))))))
  (def simd-pack-512-ub32= vpcmpeqd (unsigned-byte 32) simd-pack-512-ub32)
  (def simd-pack-512-sb32= vpcmpeqd (signed-byte 32)   simd-pack-512-sb32)
  (def simd-pack-512-ub64= vpcmpeqq (unsigned-byte 64) simd-pack-512-ub64)
  (def simd-pack-512-sb64= vpcmpeqq (signed-byte 64)   simd-pack-512-sb64)
  (def simd-pack-512-sb32> vpcmpgtd (signed-byte 32)   simd-pack-512-sb32)
  (def simd-pack-512-sb64> vpcmpgtq (signed-byte 64)   simd-pack-512-sb64))

;;; Zeroing / constant vector generation
(defknown %simd-pack-512-zero-single () (simd-pack-512 single-float)
    (flushable movable))
(define-vop (%simd-pack-512-zero-single)
  (:translate %simd-pack-512-zero-single)
  (:policy :fast-safe)
  (:results (dst :scs (single-avx512-reg)))
  (:result-types simd-pack-512-single)
  (:generator 1
...
(defknown %simd-pack-512-zero-double () (simd-pack-512 double-float)
    (flushable movable))
(define-vop (%simd-pack-512-zero-double)
  (:translate %simd-pack-512-zero-double)
  (:policy :fast-safe)
  (:results (dst :scs (double-avx512-reg)))
  (:result-types simd-pack-512-double)
  (:generator 1
...
(defknown %simd-pack-512-zero-int () (simd-pack-512 (unsigned-byte 64))
    (flushable movable))
(define-vop (%simd-pack-512-zero-int)
  (:translate %simd-pack-512-zero-int)
  (:policy :fast-safe)
  (:results (dst :scs (int-avx512-reg)))
  (:result-types simd-pack-512-ub64)
  (:generator 1
    (inst vpxord dst dst dst)))

#-sb-xc-host
(progn
  (defun %simd-pack-512-zero-single () (%simd-pack-512-zero-single))
  (defun %simd-pack-512-zero-double () (%simd-pack-512-zero-double))
  (defun %simd-pack-512-zero-int () (%simd-pack-512-zero-int)))

;;; Masked Floating Point Arithmetic (single, merge-masking)
(macrolet ((def (name inst-name)
             `(progn
                (defknown ,name ((simd-pack-512 single-float)
                                 (simd-pack-512 single-float)
                                 simd-pack-512-mask)
                    (simd-pack-512 single-float) (foldable flushable movable))
                (define-vop (,name)
                  (:translate ,name)
                  (:policy :fast-safe)
                  (:args (x :scs (single-avx512-reg) :target dst)
                         (y :scs (single-avx512-reg))
                         (mask :scs (mask-reg)))
                  (:arg-types simd-pack-512-single simd-pack-512-single simd-pack-512-mask-type)
                  (:results (dst :scs (single-avx512-reg) :from (:argument 0)))
                  (:result-types simd-pack-512-single)
                  (:generator 2
                    (unless (location= dst x)
                      (inst vmovaps dst x))
                    (inst ,inst-name dst dst y mask))))))
  (def simd-pack-512-single+-masked vaddps-masked)
  (def simd-pack-512-single--masked vsubps-masked)
  (def simd-pack-512-single*-masked vmulps-masked)
  (def simd-pack-512-single/-masked vdivps-masked))

(defknown simd-pack-512-single-sqrt-masked
    ((simd-pack-512 single-float) simd-pack-512-mask)
    (simd-pack-512 single-float) (foldable flushable movable))
(define-vop (simd-pack-512-single-sqrt-masked)
  (:translate simd-pack-512-single-sqrt-masked)
  (:policy :fast-safe)
  (:args (x :scs (single-avx512-reg) :target dst)
         (mask :scs (mask-reg)))
  (:arg-types simd-pack-512-single simd-pack-512-mask-type)
  (:results (dst :scs (single-avx512-reg) :from (:argument 0)))
  (:result-types simd-pack-512-single)
  (:generator 8
    (unless (location= dst x)
      (inst vmovaps dst x))
    (inst vsqrtps-masked dst x mask)))

;;; Masked Floating Point Arithmetic (double, merge-masking)
(macrolet ((def (name inst-name)
             `(progn
                (defknown ,name ((simd-pack-512 double-float)
                                 (simd-pack-512 double-float)
                                 simd-pack-512-mask)
                    (simd-pack-512 double-float) (foldable flushable movable))
                (define-vop (,name)
                  (:translate ,name)
                  (:policy :fast-safe)
                  (:args (x :scs (double-avx512-reg) :target dst)
                         (y :scs (double-avx512-reg))
                         (mask :scs (mask-reg)))
                  (:arg-types simd-pack-512-double simd-pack-512-double simd-pack-512-mask-type)
                  (:results (dst :scs (double-avx512-reg) :from (:argument 0)))
                  (:result-types simd-pack-512-double)
                  (:generator 2
                    (unless (location= dst x)
                      (inst vmovapd dst x))
                    (inst ,inst-name dst dst y mask))))))
  (def simd-pack-512-double+-masked vaddpd-masked)
  (def simd-pack-512-double--masked vsubpd-masked)
  (def simd-pack-512-double*-masked vmulpd-masked)
  (def simd-pack-512-double/-masked vdivpd-masked))

(defknown simd-pack-512-double-sqrt-masked
    ((simd-pack-512 double-float) simd-pack-512-mask)
    (simd-pack-512 double-float) (foldable flushable movable))
(define-vop (simd-pack-512-double-sqrt-masked)
  (:translate simd-pack-512-double-sqrt-masked)
  (:policy :fast-safe)
  (:args (x :scs (double-avx512-reg) :target dst)
         (mask :scs (mask-reg)))
  (:arg-types simd-pack-512-double simd-pack-512-mask-type)
  (:results (dst :scs (double-avx512-reg) :from (:argument 0)))
  (:result-types simd-pack-512-double)
  (:generator 8
    (unless (location= dst x)
      (inst vmovapd dst x))
    (inst vsqrtpd-masked dst x mask)))

;;; Masked Integer Arithmetic (merge-masking)
(macrolet ((def (name inst-name cl-el-type vop-type)
             `(progn
                (defknown ,name ((simd-pack-512 ,cl-el-type)
                                 (simd-pack-512 ,cl-el-type)
                                 simd-pack-512-mask)
                    (simd-pack-512 ,cl-el-type) (foldable flushable movable))
                (define-vop (,name)
                  (:translate ,name)
                  (:policy :fast-safe)
                  (:args (x :scs (int-avx512-reg) :target dst)
                         (y :scs (int-avx512-reg))
                         (mask :scs (mask-reg)))
                  (:arg-types ,vop-type ,vop-type simd-pack-512-mask-type)
                  (:results (dst :scs (int-avx512-reg) :from (:argument 0)))
                  (:result-types ,vop-type)
                  (:generator 2
                    (unless (location= dst x)
                      (inst vmovdqu64 dst x))
                    (inst ,inst-name dst dst y mask))))))
  (def simd-pack-512-ub32+-masked vpaddd-masked (unsigned-byte 32) simd-pack-512-ub32)
  (def simd-pack-512-sb32+-masked vpaddd-masked (signed-byte 32)   simd-pack-512-sb32)
  (def simd-pack-512-ub32--masked vpsubd-masked (unsigned-byte 32) simd-pack-512-ub32)
  (def simd-pack-512-sb32--masked vpsubd-masked (signed-byte 32)   simd-pack-512-sb32)
  (def simd-pack-512-ub64+-masked vpaddq-masked (unsigned-byte 64) simd-pack-512-ub64)
  (def simd-pack-512-sb64+-masked vpaddq-masked (signed-byte 64)   simd-pack-512-sb64)
  (def simd-pack-512-ub64--masked vpsubq-masked (unsigned-byte 64) simd-pack-512-ub64)
  (def simd-pack-512-sb64--masked vpsubq-masked (signed-byte 64)   simd-pack-512-sb64))

;;; Masked Logical operations (merge-masking)
(macrolet ((def (name inst-name cl-el-type vop-type)
             `(progn
                (defknown ,name ((simd-pack-512 ,cl-el-type)
                                 (simd-pack-512 ,cl-el-type)
                ...
                (define-vop (,name)
                  ...
                  (:generator 2
                    (unless (location= dst x)
                      (inst vmovdqu64 dst x))
                    (inst ,inst-name dst dst y mask))))))
  (def simd-pack-512-ub32-and-masked vpandd-masked (unsigned-byte 32) simd-pack-512-ub32)
  (def simd-pack-512-ub32-or-masked  vpord-masked  (unsigned-byte 32) simd-pack-512-ub32)
  (def simd-pack-512-ub32-xor-masked vpxord-masked (unsigned-byte 32) simd-pack-512-ub32)
  (def simd-pack-512-ub64-and-masked vpandq-masked (unsigned-byte 64) simd-pack-512-ub64)
  (def simd-pack-512-ub64-or-masked  vporq-masked  (unsigned-byte 64) simd-pack-512-ub64)
  (def simd-pack-512-ub64-xor-masked vpxorq-masked (unsigned-byte 64) simd-pack-512-ub64)
  (def simd-pack-512-and-masked      vpandq-masked (unsigned-byte 64) simd-pack-512-ub64)
  (def simd-pack-512-or-masked       vporq-masked  (unsigned-byte 64) simd-pack-512-ub64)
  (def simd-pack-512-xor-masked      vpxorq-masked (unsigned-byte 64) simd-pack-512-ub64))
