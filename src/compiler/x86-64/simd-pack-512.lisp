...
(defun zmm-zeros-ones (x)
  (declare (ignorable x))
  #+sb-xc-host (values 1 1)
  #-sb-xc-host
  (inline-vop (((x descriptor-reg) x)
               ((temp double-avx512-reg simd-pack-512-double))
               ((k mask-reg simd-pack-512-mask-type) ))
      ((ones unsigned-reg positive-fixnum)
       (zeros unsigned-reg positive-fixnum))
    (inst vmovdqu64 temp (object-slot-ea x simd-pack-512-p0-slot other-pointer-lowtag))
    (inst vptestmd k temp temp)
    (inst kmovq zeros k)
    (inst vpternlogd temp temp temp #x55)
    (inst vptestmd k temp temp)
    (inst kmovq ones k)))
...
(define-move-fun (load-int-avx512-immediate 1) (vop x y)
  ...
  (let ((x (tn-value x)))
    (multiple-value-bind (ones zeros) (zmm-zeros-ones x)
      (cond ((zerop zeros)
             (inst vpxor y y y))
            ((zerop ones)
