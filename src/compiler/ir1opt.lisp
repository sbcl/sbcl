...
(defoptimizer (values-list optimizer) ((list) node)
  (let ((use (lvar-uses list)))
    (cond ((combination-p use)
          ...
          ((and (consp use)
                (= (length use) 2))
           (flet ((transform (list-call null-ref)
                    (when (and (combination-p list-call)
                               (lvar-fun-is (combination-fun list-call) '(list))
                               (ref-p null-ref)
                               (constant-p (ref-leaf null-ref))
                               (null (constant-value (ref-leaf null-ref)))
                               (almost-immediately-used-p list null-ref)
                               (almost-immediately-used-p list list-call))
                      (change-ref-leaf (lvar-uses (combination-fun list-call))
                                       (find-free-fun 'values "values-list optimizer"))
                      ...
                      (unlink-node node)
                      t)))
             (or (transform (first use) (second use))
                 (transform (second use) (first use))))))))
