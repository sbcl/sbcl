...
(defun maybe-trace-method (gf method fun fmf-p)
  (let ((info (when (plusp (hash-table-count sb-debug::*traced-funs*))
		;; KLUDGE: testing if *TRACE-FUNS* has anything to avoid
                ;; calling METHOD-TRACE-NAME during PCL bootstrapping when
                ;; the generic-function type is not yet defined.
		(let ((m-name (method-trace-name gf method)))
