...
(define-type-method (negation :complex-union2) (type1 type2)
  (let ((not-type2 (negation-type-type type2)))
   (cond
     ...
     ((and (or (classoid-p not-type2)
               (eq not-type2 *extended-sequence-type*)
               (eq not-type2 *funcallable-instance-type*)
               (eq not-type2 *instance-type*))
              ...
              (let (rem1
                ...
                (flet ((classoid-like-p (type)
                 ...
                 (and (loop for typea in (intersection-type-types type1)
                            when (or (classoid-like-p typea)
                                     (and (negation-type-p typea)
                                          (classoid-like-p (negation-type-type typea))))
                            do
                            (let ((match (loop for typeb in (intersection-type-types type2)
                                               when (if (classoid-like-p typeb)
                                                        (and (negation-type-p typea)
