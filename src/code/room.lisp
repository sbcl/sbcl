...
;;; Relatively fast checks of where an object resides,
;;; formerly in sb-introspect.

(in-package "SB-THREAD")
(export '(heap-allocated-p stack-allocated-p))
               ...
               (all-threads
                (dolist (thread *all-threads*)
                  (when (and (thread-alive-p thread)
                             (<= (thread-control-stack-start thread) a)
                             (< a (thread-control-stack-end thread)))
                    (return thread))))))))
