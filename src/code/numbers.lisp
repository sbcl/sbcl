  ...
  `(defun ,name (x y)
     ...
     (number-dispatch ((x number) (y number))
       ...
       (((foreach fixnum bignum) ratio)
        (if (= x 0)
            (,op y)
            (dispatch-ratio (y ny dy (dispatch-type x))
              (let ((n (,op (* x dy) ny)))
                (%make-ratio n dy)))))
       ((ratio (foreach fixnum bignum))
        (if (= y 0)
            x
            (dispatch-ratio (x nx dx (dispatch-type y))
              (let ((n (,op nx (* y dx))))
                (%make-ratio n dx)))))
       ((ratio ratio)
        (dispatch-two-ratios
                        ...
                        (if (eql t1 0)
                            ...
                            (let ((g2 (gcd t1 g1)))
                              (cond ((eql g2 1)
                                     (%make-ratio t1 (* t2 dy)))
                                    ((eql g2 g1)
                                     (let* ((nn (truncate t1 g1))
                                            (nd (if (eql t2 1) ty1 (* t2 ty1))))
                                       (if (eql nd 1) nn (%make-ratio nn nd))))
                                    (t
                                     (let* ((nn (truncate t1 g2))
                                            (t3 (* (truncate g1 g2) ty1))
  ...
  (number-dispatch ((x number) (y number))
    ...
    ((ratio integer)
     (cond ((= y 1)
            x)
           ((= y -1)
            (%make-ratio (- (numerator x)) (denominator x)))
           ((= y 0)
            (error 'division-by-zero
                   :operands (list x y)
                   :operation '/))
