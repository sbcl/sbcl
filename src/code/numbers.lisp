...
;;; Do operations on ratios without consing intermediate ratios.
(defmacro with-ratio-ops (&body body)
  `(flet ((integer*ratio (x y)
            (let* ((ny (numerator y))
                   (dy (denominator y))
                   (gcd (gcd x dy)))
              (if (eql gcd 1)
                  ...
                  (let ((nn (* (truncate x gcd) ny))
                        (nd (truncate dy gcd)))
                    (values nn nd)))))
          (truncate-ratio (number numerator denominator)
            (if (= numerator 1)
                (* number denominator)
                (truncate (* number denominator)
                          numerator)))
          (ratio-ratio (nx dx ny dy)
            (if (= dy 1)
                (%make-ratio (- nx (* ny dx)) dx)
                (let ((g1 (gcd dx dy)))
                  (if (eql g1 1)
                      (%make-ratio (- (* nx dy) (* dx ny)) (* dx dy))
                      (let* ((t2 (truncate dx g1))
                             (t1 (- (* nx (truncate dy g1)) (* t2 ny)))
                             (g2 (gcd t1 g1)))
                        (cond ((eql t1 0) 0)
                              ((eql g2 1)
                               (%make-ratio t1 (* t2 dy)))
                              (t
                               (let* ((nn (truncate t1 g2))
                                 ...
                                 (if (eql nd 1)
                                     nn
                                     (%make-ratio nn nd)))))))))))
     (declare (inline integer*ratio truncate-ratio ratio-ratio))
     ,@body))
    ...
    (number-dispatch ((number real) (divisor real))
      ...
      ((ratio integer)
       (let ((numerator (numerator number))
        ...
        (if (eql divisor 1)
            (let ((q (truncate numerator denominator)))
              (values q (- number q)))
      ...
      ((ratio ratio)
       (let ((numerator (numerator number))
         ...
         (with-ratio-ops
           (multiple-value-bind (div-num div-den) (integer*ratio denominator divisor)
             (let ((q (truncate-ratio numerator
                                      div-num div-den)))
               (multiple-value-bind (rem-num rem-den) (integer*ratio q divisor)
                 (values q (ratio-ratio numerator denominator rem-num rem-den))))))))
      ((bignum fixnum)
    ...
    (number-dispatch ((number real) (divisor real))
      ...
      ((ratio ratio)
       (let ((numerator (numerator number))
         ...
         (with-ratio-ops
           (multiple-value-bind (div-num div-den) (integer*ratio denominator divisor)
             (let ((q (truncate-ratio numerator div-num div-den)))
               (multiple-value-bind (mult-num mult-den) (integer*ratio q divisor)
                 (if (if (minusp divisor)
                         (compare-two-ratios < mult-num mult-den numerator denominator)
                         (compare-two-ratios > mult-num mult-den numerator denominator))
                     (let ((q-1 (- q 1)))
                       (multiple-value-bind (mult-num mult-den) (integer*ratio q-1 divisor)
                         (values q-1 (ratio-ratio numerator denominator mult-num mult-den))))
                     (values q (ratio-ratio numerator denominator mult-num mult-den)))))))))
      ((bignum fixnum)
