...
main(int argc, char *argv[], char *envp[])
{
    ...
    if (linked_in_core_data.found) {
        ...
        SetSymbolValue(ALLOCATION_POINTER,
                       (lispobj)linked_in_core_data.dynamic_space_free_offset,
                       0);
#else
        dynamic_space_free_pointer =
               linked_in_core_data.dynamic_space_free_offset;
#endif
...
#               endif
    ...
    if (linked_in_core_data.found) {
        ...
        immobile_space_coreparse(
            linked_in_core_data.immobile_fixedobj_space_offset,
            linked_in_core_data.immobile_fixedobj_space_size);
        immobile_space_coreparse(
            linked_in_core_data.immobile_varyobj_space_offset,
