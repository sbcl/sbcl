...
#else
// this is subtly wrong and I do not care enough to fix it.
// why is it wrong? Because the autogenerated "src/runtime/genesis/thread.h"
// assumes that every field takes up 1 word (or more). However the os_thread field
// on a 64-bit build with a 4-byte pid_t is undersized by 4 bytes.
// Thanks to ABI alignment requirements, the C compiler is obliged to insert
// 4 padding bytes (which are inaccessible).
// There's another problem though, which is that in error messages we often
// want to cast thread->os_thread to a (void*) so that it can be formatted
// using "%p" (which is technically wrong anyway). But casting a 4-byte
// integer to a pointer gets
//   "cast to 'void *' from smaller integer type 'os_thread_t'"
// which is how I discovered this problem in the first place.
// I think the right solution would be to define the field in 'struct thread'
// as having type uword_t, not os_thread_t.
