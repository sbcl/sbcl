  ...
  (multiple-value-bind (i i0 i-1
    ...
    (loop for expected in (list '(1f0 2f0 3f0 4f0 5f0 6f0 7f0)
          ...
          do (assert (every #'eql expected
                            (multiple-value-list (%simd-pack-256-singles pack)))))
    (loop for expected in (list '(1d0 2d0 3d0 4d0)
          ...
          do (assert (every #'eql expected
                            (multiple-value-list (%simd-pack-256-doubles pack)))))))
  ...
  (let (tmp-fasl)
    (unwind-protect
         (progn
           ...
           (let ((*pack* nil))
             (load tmp-fasl)
             (assert (typep *pack* '(sb-ext:simd-pack-256 (unsigned-byte 64))))
    ...
    (unwind-protect
         (progn
           ...
           (let ((*pack* nil))
             ...
             (assert (equal (multiple-value-list (%simd-pack-256-singles *pack*))
    ...
    (unwind-protect
         (progn
           ...
           (let ((*pack* nil))
             ...
             (assert (equal (multiple-value-list (%simd-pack-256-doubles *pack*))
                            '(1d0 2d0 3d0 4d0)))))
      (when tmp-fasl (delete-file tmp-fasl))
      (delete-file *tmp-filename*))))
...
(with-test (:name (simd-pack-256 subtypep :smoke))
  (assert-tri-eq t t (subtypep '(simd-pack-256 (unsigned-byte 8)) 'simd-pack-256))
  (assert-tri-eq t t (subtypep '(simd-pack-256 (unsigned-byte 16)) 'simd-pack-256))
  (assert-tri-eq t t (subtypep '(simd-pack-256 (unsigned-byte 32)) 'simd-pack-256))
  (assert-tri-eq t t (subtypep '(simd-pack-256 (unsigned-byte 64)) 'simd-pack-256))
  (assert-tri-eq t t (subtypep '(simd-pack-256 (signed-byte 8)) 'simd-pack-256))
  (assert-tri-eq t t (subtypep '(simd-pack-256 (signed-byte 16)) 'simd-pack-256))
  (assert-tri-eq t t (subtypep '(simd-pack-256 (signed-byte 32)) 'simd-pack-256))
  (assert-tri-eq t t (subtypep '(simd-pack-256 (signed-byte 64)) 'simd-pack-256))
  (assert-tri-eq t t (subtypep '(simd-pack-256 single-float) 'simd-pack-256))
  (assert-tri-eq t t (subtypep '(simd-pack-256 double-float) 'simd-pack-256))
  (assert-tri-eq nil t (subtypep 'simd-pack-256 '(simd-pack-256 (unsigned-byte 64))))
  (assert-tri-eq nil t (subtypep 'simd-pack-256 '(simd-pack-256 single-float)))
  (assert-tri-eq nil t (subtypep 'simd-pack-256 '(simd-pack-256 double-float)))
  (assert-tri-eq t t (subtypep '(simd-pack-256 (unsigned-byte 64))
                               '(or (simd-pack-256 (unsigned-byte 64)) (simd-pack-256 single-float))))
  (assert-tri-eq t t (subtypep '(simd-pack-256 (unsigned-byte 64))
                               '(or (simd-pack-256 (unsigned-byte 64)) (simd-pack-256 double-float))))
  (assert-tri-eq nil t (subtypep '(simd-pack-256 (unsigned-byte 64))
                                 '(or (simd-pack-256 single-float) (simd-pack-256 double-float))))
  (assert-tri-eq nil t (subtypep '(or (simd-pack-256 (unsigned-byte 64)) (simd-pack-256 single-float))
                                 '(simd-pack-256 (unsigned-byte 64))))
  (assert-tri-eq nil t (subtypep '(or (simd-pack-256 (unsigned-byte 64)) (simd-pack-256 double-float))
                                 '(simd-pack-256 (unsigned-byte 64))))
...
(with-test (:name (simd-pack-256 :ctype-unparse :smoke))
  (flet ((unparsed (s) (sb-kernel:type-specifier (sb-kernel:specifier-type s))))
    ...
    (assert (equal (unparsed '(simd-pack-256 (unsigned-byte 8))) '(simd-pack-256 (unsigned-byte 8))))
    (assert (equal (unparsed '(simd-pack-256 (unsigned-byte 16))) '(simd-pack-256 (unsigned-byte 16))))
    (assert (equal (unparsed '(simd-pack-256 (unsigned-byte 32))) '(simd-pack-256 (unsigned-byte 32))))
    (assert (equal (unparsed '(simd-pack-256 (unsigned-byte 64))) '(simd-pack-256 (unsigned-byte 64))))
    (assert (equal (unparsed '(simd-pack-256 (signed-byte 8))) '(simd-pack-256 (signed-byte 8))))
    (assert (equal (unparsed '(simd-pack-256 (signed-byte 16))) '(simd-pack-256 (signed-byte 16))))
    (assert (equal (unparsed '(simd-pack-256 (signed-byte 32))) '(simd-pack-256 (signed-byte 32))))
    (assert (equal (unparsed '(simd-pack-256 (signed-byte 64))) '(simd-pack-256 (signed-byte 64))))
    ...
    (assert (equal (unparsed '(or (simd-pack-256 (unsigned-byte 64)) (simd-pack-256 double-float)))
                   ;; depends on *SIMD-PACK-ELEMENT-TYPES* order
                   '(or (simd-pack-256 double-float) (simd-pack-256 (unsigned-byte 64)))))
