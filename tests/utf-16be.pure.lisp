...
;;;; big-endian UTF-16, using test machinery which does not have side
             ...
             `(let ((bytes ,bytes))
                ...
                (symbol-macrolet ((name ,id))
                  (macrolet ((test (inxf expected)
                    ...
                    ,@body)))))
  (with-input-bytes ((:input :invalid) (list #x00 #x35 #xff #xff #x00 #x37))
    (test :utf-16be '(#\5 #\7))
    (test (:utf-16be :replacement #\?) '(#\5 #\? #\7)))
  (with-input-bytes ((:input :multiple-invalid) (list #x00 #x35 #xff #xff #xff #xff #x00 #x37))
    (test :utf-16be '(#\5 #\7))
    (test (:utf-16be :replacement #\?) '(#\5 #\? #\? #\7)))
