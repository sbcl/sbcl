...
(macrolet ((test (inxf expected &environment env)
  ...
  (with-test-file ((file-position :replacement character) '(#x35 #x0d #x0a #xff #x37 #xff #x0d #x0a))
    (test (:ascii :replacement #\?) (loop for i from 0 to 8 collect i))
    (test (:ascii :newline :lf :replacement #\?) (loop for i from 0 to 8 collect i))
    (test (:ascii :newline :cr :replacement #\?) (loop for i from 0 to 8 collect i))
    (test (:ascii :newline :crlf :replacement #\?) '(0 1 3 4 5 6 8)))
  (with-test-file ((file-position :replacement string) '(#x35 #x0d #x0a #xff #x37 #xff #x0d #x0a))
    (test (:ascii :replacement "??") '(0 1 2 3 4 4 5 6 6 7 8))
    (test (:ascii :newline :lf :replacement "??") '(0 1 2 3 4 4 5 6 6 7 8))
    (test (:ascii :newline :cr :replacement "??")  '(0 1 2 3 4 4 5 6 6 7 8))
    (test (:ascii :newline :crlf :replacement "??") '(0 1 3 4 4 5 6 6 8))))
