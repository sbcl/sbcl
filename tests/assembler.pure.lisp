...
(test-util:with-test (:name :x86-64-assembler-peephole-optimizations :skipped-on (not :x86-64))
  ;; 1. Condition code implication lattice in branch-branch-implication
  (flet ((implies (c1 c2)
           (sb-x86-64-asm::branch-branch-implication
            (if (eq c1 :always) :always (sb-x86-64-asm::encoded-condition c1))
            (if (eq c2 :always) :always (sb-x86-64-asm::encoded-condition c2)))))
    ;; Identity & always
    (assert (eq (implies :e :e) :taken))
    (assert (eq (implies :e :always) :taken))
    ;; Opposite condition
    (assert (eq (implies :e :ne) :not-taken))
    ;; :e implications
    (assert (eq (implies :e :be) :taken))
    (assert (eq (implies :e :le) :taken))
    (assert (eq (implies :e :a) :not-taken))
    (assert (eq (implies :e :b) :not-taken))
    (assert (eq (implies :e :g) :not-taken))
    (assert (eq (implies :e :l) :not-taken))
    ;; :b implications
    (assert (eq (implies :b :be) :taken))
    (assert (eq (implies :b :ne) :taken))
    (assert (eq (implies :b :ae) :not-taken))
    (assert (eq (implies :b :e) :not-taken))
    ;; :a implications
    (assert (eq (implies :a :ae) :taken))
    (assert (eq (implies :a :ne) :taken))
    (assert (eq (implies :a :be) :not-taken))
    (assert (eq (implies :a :b) :not-taken))
    ;; :l implications
    (assert (eq (implies :l :le) :taken))
    (assert (eq (implies :l :ne) :taken))
    (assert (eq (implies :l :ge) :not-taken))
    (assert (eq (implies :l :g) :not-taken))
    ;; :g implications
    (assert (eq (implies :g :ge) :taken))
    (assert (eq (implies :g :ne) :taken))
    (assert (eq (implies :g :le) :not-taken))
    (assert (eq (implies :g :l) :not-taken)))

  ;; 2. Section peephole transformations
  (flet ((collect-ops (section)
           (let ((res nil))
             (do ((s (sb-assem::section-start section) (sb-assem::stmt-next s)))
                 ((null s) (nreverse res))
               (push (list (sb-assem::stmt-op s)
                           (sb-assem::stmt-prefix s)
                           (length (sb-assem::ensure-list (sb-assem::stmt-labels s))))
                     res)))))
    ;; A. 64-bit self move eliminated, 32-bit self move preserved
    (let* ((stream (sb-assem:make-asmstream))
      ...
      (sb-assem:assemble (section)
        (inst mov rax-tn rax-tn)          ; 64-bit: eliminated
        (inst mov :dword rax-tn rax-tn)   ; 32-bit: preserved
        (inst ret))
      (sb-assem::combine-instructions section)
      (let ((ops (collect-ops section)))
        ;; IGNORE, MOV (:dword prefix 6), RET
        (assert (= (length ops) 3))
        (assert (eq (caar ops) :ignore))
        (assert (eq (caadr ops) 'sb-x86-64-asm::mov))
        (assert (eq (cadadr ops) 6)) ; :dword prefix
        (assert (eq (caaddr ops) 'sb-x86-64-asm::ret))))

    ;; B. CMP reg, 0 -> TEST reg, reg
    (let* ((stream (sb-assem:make-asmstream))
           (section (sb-assem::asmstream-code-section stream)))
      (setf sb-assem::*asmstream* stream)
      (sb-assem:assemble (section)
        (inst cmp rax-tn 0)
        (inst cmp :dword rax-tn 0)
        (inst ret))
      (sb-assem::combine-instructions section)
      (let ((ops (collect-ops section)))
        ;; IGNORE, TEST, TEST (:dword prefix 6), RET
        (assert (= (length ops) 4))
        (assert (eq (caadr ops) 'sb-x86-64-asm::test))
        (assert (eq (caaddr ops) 'sb-x86-64-asm::test))
        (assert (eq (car (fourth ops)) 'sb-x86-64-asm::ret))))

    ;; C. ALU + TEST redundancy elimination
    (let* ((stream (sb-assem:make-asmstream))
           (section (sb-assem::asmstream-code-section stream))
           (target (sb-assem:gen-label)))
      (setf sb-assem::*asmstream* stream)
      (sb-assem:assemble (section)
        (inst add rax-tn rbx-tn)
        (inst test rax-tn rax-tn)
        (inst jmp :e target)
        (sb-assem:emit-label target)
        (inst ret))
      (sb-assem::combine-instructions section)
      (let ((ops (collect-ops section)))
        ;; IGNORE, ADD, JMP, RET (TEST eliminated!)
        (assert (= (length ops) 4))
        (assert (eq (caadr ops) 'sb-x86-64-asm::add))
        (assert (eq (caaddr ops) 'sb-x86-64-asm::jmp))
        (assert (eq (car (fourth ops)) 'sb-x86-64-asm::ret))))

    ;; D. Unconditional jump to immediate fallthrough eliminated
    (let* ((stream (sb-assem:make-asmstream))
           (section (sb-assem::asmstream-code-section stream))
           (fallthrough (sb-assem:gen-label)))
      (setf sb-assem::*asmstream* stream)
      (sb-assem:assemble (section)
        (inst jmp fallthrough)
        (sb-assem:emit-label fallthrough)
        (inst ret))
      (sb-assem::combine-instructions section)
      (let ((ops (collect-ops section)))
        ;; IGNORE, RET (JMP eliminated, RET has label)
        (assert (= (length ops) 2))
        (assert (eq (caadr ops) 'sb-x86-64-asm::ret))
        (assert (= (third (second ops)) 1))))

    ;; E. Transitive move forwarding and circular move deletion
    ;; MOV rbx, rax ; MOV rdx, rbx ; MOV rax, rdx
    ;; -> MOV rbx, rax ; MOV rdx, rax (3rd move deleted by symmetric move elim)
    (let* ((stream (sb-assem:make-asmstream))
           (section (sb-assem::asmstream-code-section stream)))
      (setf sb-assem::*asmstream* stream)
      (sb-assem:assemble (section)
        (inst mov rbx-tn rax-tn)
        (inst mov rdx-tn rbx-tn)
        (inst mov rax-tn rdx-tn)
        (inst ret))
      (sb-assem::combine-instructions section)
      (let ((ops (collect-ops section)))
        ;; IGNORE, MOV, MOV, RET (3 moves reduced to 2)
        (assert (= (length ops) 4))
        (assert (eq (caadr ops) 'sb-x86-64-asm::mov))
    ...
    (let* ((stream (sb-assem:make-asmstream))
      ...
      (sb-assem::combine-instructions section)
      (let ((ops (collect-ops section)))
        ;; IGNORE, SHL, JMP, RET (TEST eliminated!)
        (assert (= (length ops) 4))
        (assert (eq (caadr ops) 'sb-x86-64-asm::shl))
        (assert (eq (caaddr ops) 'sb-x86-64-asm::jmp))
        (assert (eq (car (fourth ops)) 'sb-x86-64-asm::ret))))

    ;; H. Flag-dead MOV 0 -> XOR zeroization
    (let* ((stream (sb-assem:make-asmstream))
           (section (sb-assem::asmstream-code-section stream)))
      (setf sb-assem::*asmstream* stream)
      (sb-assem:assemble (section)
      ...
      (sb-assem::combine-instructions section)
      (let ((ops (collect-ops section)))
        ;; IGNORE, XOR (:dword prefix 6), ADD, RET
        (assert (= (length ops) 4))
        (assert (eq (caadr ops) 'sb-x86-64-asm::xor))
        (assert (eq (cadadr ops) 6))
        (assert (eq (caaddr ops) 'sb-x86-64-asm::add))
        (assert (eq (car (fourth ops)) 'sb-x86-64-asm::ret))))

    ;; I. Shift coalescing (SHL + SHL)
    (let* ((stream (sb-assem:make-asmstream))
           (section (sb-assem::asmstream-code-section stream)))
      (setf sb-assem::*asmstream* stream)
      (sb-assem:assemble (section)
      ...
      (sb-assem::combine-instructions section)
      (let ((ops (collect-ops section)))
        ;; IGNORE, SHL, RET (2 shifts merged into 1)
        (assert (= (length ops) 3))
        (assert (eq (caadr ops) 'sb-x86-64-asm::shl))
        (assert (eq (caaddr ops) 'sb-x86-64-asm::ret))))

    ;; J. Jump-over-jump branch inversion
    ;; Jcc L_fallthrough ; JMP L_target ; L_fallthrough:
    ;; -> J!cc L_target ; L_fallthrough:
    (let* ((stream (sb-assem:make-asmstream))
           (section (sb-assem::asmstream-code-section stream))
           (l1 (sb-assem:gen-label))
           (l2 (sb-assem:gen-label)))
      (setf sb-assem::*asmstream* stream)
      (sb-assem:assemble (section)
        (inst cmp rax-tn rbx-tn)
        (inst jmp :ne l1)
        (inst jmp l2)
      ...
      (sb-assem::combine-instructions section)
      (let ((ops (collect-ops section)))
        ;; IGNORE, CMP, JMP (:e), MOV (label l1), RET (label l2)
        (assert (= (length ops) 5))
        (assert (eq (caadr ops) 'sb-x86-64-asm::cmp))
        (assert (eq (caaddr ops) 'sb-x86-64-asm::jmp))
        (assert (eq (car (fourth ops)) 'sb-x86-64-asm::mov))
        (assert (eq (car (fifth ops)) 'sb-x86-64-asm::ret))))

    ;; K. Store-to-Load forwarding
    ;; MOV [rbp-48], rdi ; MOV rax, [rbp-48] -> MOV [rbp-48], rdi ; MOV rax, rdi
    (let* ((stream (sb-assem:make-asmstream))
           (section (sb-assem::asmstream-code-section stream)))
      (setf sb-assem::*asmstream* stream)
      (sb-assem:assemble (section)
        (inst mov (ea -48 rbp-tn) rdi-tn)
        (inst mov rax-tn (ea -48 rbp-tn))
        (inst ret))
      (sb-assem::combine-instructions section)
      (let ((ops (collect-ops section)))
        ;; IGNORE, MOV (store), MOV (reg-to-reg copy), RET
        (assert (= (length ops) 4))
        (assert (eq (caadr ops) 'sb-x86-64-asm::mov))
        (assert (eq (caaddr ops) 'sb-x86-64-asm::mov))
        (assert (eq (car (fourth ops)) 'sb-x86-64-asm::ret))))

    ;; L. Load-after-Load forwarding
    ;; MOV rbx, [rbp-56] ; MOV rcx, [rbp-56] -> MOV rbx, [rbp-56] ; MOV rcx, rbx
    (let* ((stream (sb-assem:make-asmstream))
           (section (sb-assem::asmstream-code-section stream)))
      (setf sb-assem::*asmstream* stream)
      (sb-assem:assemble (section)
        (inst mov rbx-tn (ea -56 rbp-tn))
        (inst mov rcx-tn (ea -56 rbp-tn))
        (inst ret))
      (sb-assem::combine-instructions section)
      (let ((ops (collect-ops section)))
        ;; IGNORE, MOV (load), MOV (reg-to-reg copy), RET
        (assert (= (length ops) 4))
        (assert (eq (caadr ops) 'sb-x86-64-asm::mov))
        (assert (eq (caaddr ops) 'sb-x86-64-asm::mov))
        (assert (eq (car (fourth ops)) 'sb-x86-64-asm::ret))))

    ;; M. MOV + ADD -> LEA fusion
    ;; MOV rsi, rdx ; ADD rsi, 16 -> LEA rsi, [rdx+16]
    (let* ((stream (sb-assem:make-asmstream))
           (section (sb-assem::asmstream-code-section stream)))
      (setf sb-assem::*asmstream* stream)
      (sb-assem:assemble (section)
        (inst mov rsi-tn rdx-tn)
        (inst add rsi-tn 16)
        (inst ret))
      (sb-assem::combine-instructions section)
      (let ((ops (collect-ops section)))
        ;; IGNORE, LEA, RET (MOV eliminated, ADD converted to LEA)
        (assert (= (length ops) 3))
        (assert (eq (caadr ops) 'sb-x86-64-asm::lea))
        (assert (eq (caaddr ops) 'sb-x86-64-asm::ret))))))
