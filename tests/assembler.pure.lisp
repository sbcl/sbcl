...
(test-util:with-test (:name :apx-instruction-encodings :skipped-on (not :x86-64))
  (flet ((get-apx-gpr (size number)
           (let ((r (copy-structure (sb-x86-64-asm::get-gpr :qword 0))))
             (setf (sb-x86-64-asm::reg-id r)
                   (logior (ash number 3)
                           (ash (position size #(:qword :dword :word :byte)) 1)))
             r)))
    (let ((r0   (get-apx-gpr :qword 0))
          (r1   (get-apx-gpr :qword 1))
          (r2   (get-apx-gpr :qword 2))
          (r16  (get-apx-gpr :qword 16))
          (r17  (get-apx-gpr :qword 17))
          (r18  (get-apx-gpr :qword 18))
          (r31  (get-apx-gpr :qword 31))
          (r16d (get-apx-gpr :dword 16))
          (r17d (get-apx-gpr :dword 17))
          (r18d (get-apx-gpr :dword 18)))
      (declare (ignorable r31))
    (flet ((check-bytes (inst expected-bytes)
             (let ((seg (sb-assem:make-segment)))
               ...
               (assert (equal (coerce (subseq (sb-assem::segment-buffer seg) 0 (sb-assem::segment-current-posn seg)) 'list)
                              expected-bytes)))))
      ;; 3-operand NDD ALU (ADD, SUB, XOR, AND, OR)
      (check-bytes `(add-ndd ,r16 ,r17 ,r18) '(#x62 #xEC #xFC #x10 #x01 #xD1))
      (check-bytes `(sub-ndd ,r16 ,r17 ,r18) '(#x62 #xEC #xFC #x10 #x29 #xD1))
      (check-bytes `(and-ndd ,r16 ,r17 ,r18) '(#x62 #xEC #xFC #x10 #x21 #xD1))
      (check-bytes `(or-ndd  ,r16 ,r17 ,r18) '(#x62 #xEC #xFC #x10 #x09 #xD1))
      (check-bytes `(xor-ndd ,r16 ,r17 ,r18) '(#x62 #xEC #xFC #x10 #x31 #xD1))
      ;; NDD with legacy GPRs (RAX, RCX, RDX)
      (check-bytes `(add-ndd ,r0 ,r1 ,r2)    '(#x62 #xF4 #xFC #x18 #x01 #xD1))
      ;; 32-bit DWORD NDD
      (check-bytes `(add-ndd ,r16d ,r17d ,r18d) '(#x62 #xEC #x7C #x10 #x01 #xD1))
