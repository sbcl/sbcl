...
(test-util:with-test (:name :apx-extended-registers-and-instructions)
  (let ((r16 (sb-x86-64-asm::get-gpr :qword 16))
        ...
        (r16d (sb-x86-64-asm::get-gpr :dword 16))
        (r17d (sb-x86-64-asm::get-gpr :dword 17))
        (r16w (sb-x86-64-asm::get-gpr :word 16))
        (r17w (sb-x86-64-asm::get-gpr :word 17))
        (r16b (sb-x86-64-asm::get-gpr :byte 16))
        (r17b (sb-x86-64-asm::get-gpr :byte 17)))
    ;; 1. REX2 prefix (0xD5) with R16-R31
    (test-assemble `(mov ,r16 ,r17)
                   "D5588BC1         MOV R16, R17")
    (test-assemble `(add ,r31 ,r16)
                   "D55901C7         ADD R31, R16")
    (test-assemble `(mov ,r16d ,r17d)
                   "D5508BC1         MOV R16D, R17D")
    (test-assemble `(mov ,r16w ,r17w)
                   "66D5508BC1       MOV R16W, R17W")
    (test-assemble `(mov ,r16b ,r17b)
                   "D5508AC1         MOV R16B, R17B")
    ;; 2. APX EVEX NDD, PUSH2, POP2, CCMP, CTEST, CFCMOV
    (flet ((check-bytes (inst expected-bytes)
             (let ((seg (sb-assem:make-segment)))
               (sb-assem:assemble (seg nil)
                 (apply #'sb-assem:inst* (car inst) (cdr inst)))
               (assert (equal (coerce (subseq (sb-assem::segment-buffer seg) 0 (sb-assem::segment-current-posn seg)) 'list)
                              expected-bytes)))))
      (check-bytes `(add-ndd ,r16 ,r17 ,r18) '(#x62 #xEC #xFC #x10 #x01 #xD1))
      (check-bytes `(push2 ,r16 ,r17) '(#x62 #xFC #x7C #x10 #xFF #xF1))
      (check-bytes `(pop2 ,r16 ,r17) '(#x62 #xFC #x7C #x10 #x8F #xC1))
      (check-bytes `(ccmp :e ,r16 ,r17 0) '(#x62 #xEC #x84 #x04 #x39 #xC8))
      (check-bytes `(ctest :ne ,r16 ,r17 0) '(#x62 #xEC #x84 #x05 #x85 #xC8))
      (check-bytes `(cfcmov :e ,r16 ,r17) '(#x62 #xEC #xFC #x08 #x44 #xC1)))))
